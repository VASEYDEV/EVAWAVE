...
describe("lint", () => {
  it("registers every S1 rule once, in id order", () => {
    const ids = RULES.map((rule) => rule.id);
    expect(ids).toEqual([...ids].sort());
    expect(ids).toEqual(["BG-1", "BG-2", "BT-1", "CP-1", "CP-2", "CP-3", "LN-1", "ML-1", "ML-2", "ML-3", "ML-4", "PB-1", "PB-2", "RB-1", "RB-2", "RB-3", "SC-1", "SC-2", "TQ-1"]);
  });
