...
# musicspec core — path-scoped rules (visible date marker: 2026-09-26)
...
2. Serializers are deterministic functions of (MusicSpec, EngineProfile, Catalog) per `docs/SPEC.md` §2. Same inputs, same bytes. No Date.now(), no randomness, no I/O.
...
7. The Jinn v1.2 golden test (`docs/SPEC.md` §3, S1) must pass after any serializer or lint change. It asserts byte equality with the three fields in `docs/evawave/reference/jinn-v1.2-condensed.md`, read at test time. The Jinn files are ground truth: never write or edit them (ADR 0004).
