...
- S6, songs and variants (SPEC §3, ADR 0006):
  ...
  - The composer's Song panel (save as a new song, save, freeze as the next variant), a Songs section in `/library`, and `/songs/[id]` with each variant's parent, coverage and field diff. Any variant opens in the composer, and the next freeze forks from it. Opening over unsaved work (a changed spec or base variant), and deleting a song, ask twice. `/library`, the song page and the composer's Song panel follow the account that is signed in now, and read every song, variant and take a page at a time by key, past the server's row limit. A song is read before its variants; a save or freeze attaches only to the copy it began on; opening a song stays put, with a message, when the browser will not store the copy.
