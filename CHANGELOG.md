...
- S5, audio import, tempo tools and iconography (SPEC §3):
  ...
  - `/import`: analysis in a Web Worker (`public/workers/analysis.worker.js`, generated by `scripts/build-analysis-worker.mjs`), file picker and drag and drop, OPFS storage once a profile is saved to the library, kept per account (and removal when its `/library` record is deleted), review diff, profile creation, save to the library, and download.
