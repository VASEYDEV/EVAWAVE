...
- S6, songs and variants (SPEC §3, ADR 0006):
  ...
  - The composer's Song panel (save as a new song, save, freeze as the next variant), a Songs section in `/library`, and `/songs/[id]` with each variant's parent, coverage and field diff. Any variant opens in the composer, and the next freeze forks from it. Opening over unsaved work (a changed spec or base variant), and deleting a song, ask twice; the delete names the variants and every take logged on them, and removes nothing if the song was saved or frozen since the list loaded (the list reads songs before their variant counts, so a count never predates its revision). A variant's snapshot leaves out the working copy's patches. `/library`, the song page and the composer's Song panel follow the account that is signed in now, and read every song, variant and take a page at a time by key, past the server's row limit. A song is read before its variants; a save or freeze attaches only to the copy it began on; opening a song stays put, with a message, when the browser will not store the copy.
