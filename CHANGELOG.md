...
- S5, audio import, tempo tools and iconography (SPEC §3):
  ...
  - `/import`: files up to 100 MiB and recordings up to 10 minutes (by their metadata), or 20 channel-minutes by the channel count the header declares (`channels.ts`: WAV, AIFF, FLAC, Ogg, MP3, ADTS, MP4, CAF, Matroska; 32 channels assumed when it does not say), refused before they are read, as are recordings whose length the metadata does not give, and decoded at a fixed 48 kHz; profiles hold only the accepted fields (`StyleProfileSpec`); analysis in a Web Worker (`public/workers/analysis.worker.js`, generated by `scripts/build-analysis-worker.mjs`), file picker and drag and drop, OPFS storage once a profile is saved to the library, kept per account (and removal when its `/library` record is deleted, here or, on the next `/library` visit, from another device), review diff, profile creation, save to the library, and download.
